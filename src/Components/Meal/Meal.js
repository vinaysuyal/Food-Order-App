...
const Meal = (props) => {
  ...
  const onAddCartItem = (reps) => {
    cartContext.dispatchCartData({
      type: "EditCart",
      newCartData: props.mealData,
      reps,
    });
  };
  const matchItem = cartContext.cartData.find(
    (item) => item.id === props.mealData.id
  );
  return (
    <Card className="meals-card">
      ...
      <div className="pricingAndCart">
        ...
        {!props.isConfirmedOrder && (
          <AddRemoveForm
            onChange={onAddCartItem}
            value={matchItem ? matchItem.reps : 0}
          />
        )}
      </div>
    </Card>
